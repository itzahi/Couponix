<%@ page language="java" contentType="text/html; charset=windows-1255"
 ...
 <!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
          <%-- this JSP page  is managed by the Admin, responsible for adding proper Category 
		Adds for any Category: ID, name and image 
	   ...
	   <head>   
  		...
  		%>
              ...
              <title>Add New Category - Couponic Admin</title>
 ...
 <!-- Collect the nav links, forms, and other content for toggling -->
    ...
    <ul class="nav navbar-nav navbar-right navbar-user">
      ...
      <div id="page-wrapper">
        ...
        <div class="row">
          <div class="col-lg-12">
            <h1>Update Category<small></small></h1>
            <ul class="breadcrumb">
              <li><i class="icon-dashboard"></i><a href="${pageContext.request.contextPath}/couponAdmin/couponAdmin.jsp"> Dashboard</a></li>
              <li><a href="${pageContext.request.contextPath}/couponAdmin/categoryManagement.jsp"><i class="icon-file-alt"></i> Back To Category Management</a></li>
            </ul>
          </div>
        </div><!-- /.row -->
		<div class="row">
          <div class="col-lg-6">
            <form role="form" action="${pageContext.request.contextPath}/CouponsController/adminController" name="updateCategoryForm"  method="POST" onsubmit="return checkForm();" >
	  		  <input type="hidden" name="controller" class="form-control" value="updateCategory" >
		      ...
		      <div class="form-group">
                ...
                <input type="text" name="categoryImage" class="form-control" placeholder="Enter image URL">
              </div>
  ...
  function checkForm()
  {
	    var id = document.updateCategoryForm.id.value;                       //ID describes the ID of the Category and is unique for every Category 
	    var categoryName = document.updateCategoryForm.categoryName.value;	  //categoryName describes the name of the Category adding to a database
	    var categoryImage = document.updateCategoryForm.categoryImage.value;	//categoryImage describes the image/icon of  Category adding by url to  database
		...
		var flag = true;													//flag describes if  all details are valid or not 
			...
			if ((isEmpty(id)) == true ||(checkString(okNum, id)) == false)
			{
				 ...
				 document.updateCategoryForm.id.value;
				 flag = false;
			}
			
			//check categoryName field is empty  and Displaying Appropriate message to client 
			if ((isEmpty(categoryName)) == true)
			{
				 alert('categoryName :you need to insert a valid categoryName');
				 document.updateCategoryForm.categoryName.value;
				 flag = false;
			}
			
			//check categoryName field is empty or url not valid and Displaying Appropriate message to client
			if ((isEmpty(categoryImage)) == true  || isValidURL(categoryImage) == false)
			{
				 alert('Image : you need to insert valid Image');
				 document.updateCategoryForm.categoryImage.value;
				 flag = false;
			}
